...
long double newton(long double e1, long double e2, long double apx,  expr ex, expr dex, int max){
    ...
    while(k <= max){
        x1 = x0 - (ex.avaliar(x0)/dex.avaliar(x0));
        fx1 = ex.avaliar(x1);
...
int receber(expr* ex, expr* dex, long double* e1, long double* e2, long double* apx, int* max){
    ...
    cin >> s;
    if(s == "Sair") return 1;
    cout << "Insira a expressão da função derivada f'(x):\n";
    cin >> s2;
